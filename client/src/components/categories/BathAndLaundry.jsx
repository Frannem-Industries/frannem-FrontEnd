...
const BathAndLaundry = ({ products = [] }) => {
  const displayProducts = products.slice(0, 4);
  
  return (
    <div className="w-full max-w-7xl">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold">Bath And Laundry</h2>
        <Link to="/category/bath-and-laundry" className="text-primary-blue hover:underline">
          View All
        </Link>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {displayProducts.length > 0 ? (
          ...
          <div className="col-span-full text-center py-8 text-gray-500">
            No Laundry products available at the moment.
          </div>
        )}
      </div>
    </div>
